...
const EXTERNAL_URL = process.env.EXTERNAL_URL; // required to forward messages

if (!EXTERNAL_URL) {
  console.error('Missing EXTERNAL_URL environment variable');
  // We still start but readiness will fail
}
...
app.get('/ready', (req, res) => {
  ...
  if (!EXTERNAL_URL) return res.status(503).json({ ready: false, reason: 'external-url-missing' });
  ...
  (async () => {
    ...
    for await (const m of sub) {
      try {
        ...
        // Best-effort forward to external service
        try {
