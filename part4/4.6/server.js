...
app.put('/api/todos/:id', async (req, res) => {
  try {
    ...
    console.error('Error proxying PUT /api/todos/:id:', err.message);
    res.status(500).json({ error: 'Failed to update todo in backend' });
  }
});
      ...
      <script>
        ...
        function renderTodos() {
          ...
          todos.forEach(todo => {
            ...
            li.innerHTML = '<input type="checkbox" ' + (todo.done ? 'checked' : '') + ' onchange="toggleTodo(' + todo.id + ')">' +
        ...
        async function toggleTodo(id) {
          ...
          try {
            ...
            if (idx >= 0) todos[idx] = updated; else todos.push(updated);
            renderTodos();
          } catch (err) {
