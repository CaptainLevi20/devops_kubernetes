...
async function connectNats() {
  try {
    ...
    natsClient.closed().then(() => { natsConnected = false; }).catch(() => {});
  } catch (err) {
    console.warn('Could not connect to NATS, continuing without event publishing:', err.message || err);
    natsConnected = false;
    natsClient = null;
  }
}
...
app.post('/todos', async (req, res) => {
  ...
  try {
    ...
    try {
      if (natsConnected && natsClient) {
        const payload = { event: 'created', id: created.id, text: created.text, done: !!created.completed, ts: new Date().toISOString() };
        natsClient.publish(NATS_SUBJECT, sc.encode(JSON.stringify(payload)));
      }
    } catch (err) {
      console.warn('Failed to publish NATS event:', err && err.message ? err.message : err);
    }
...
app.put('/todos/:id', async (req, res) => {
  ...
  try {
    ...
    console.log(okLine);
    appendLog(okLine);
    // Publish update event to NATS (best-effort)
    try {
      if (natsConnected && natsClient) {
        const payload = { event: 'updated', id: row.id, text: row.text, done: !!row.completed, ts: new Date().toISOString() };
        natsClient.publish(NATS_SUBJECT, sc.encode(JSON.stringify(payload)));
      }
    } catch (err) {
      console.warn('Failed to publish NATS event:', err && err.message ? err.message : err);
...
(async () => {
  await initDb();
  // Attempt to connect to NATS but don't block server startup if unavailable
  await connectNats();
  app.listen(PORT, () => {
    console.log(`todo-backend listening on ${PORT}`);
  });
  process.on('SIGINT', async () => { if (natsClient) await natsClient.close(); process.exit(0); });
  process.on('SIGTERM', async () => { if (natsClient) await natsClient.close(); process.exit(0); });
})();
